...
const PropertyList = () => {
    ...
    const fetchProperties = async(page = 1) => {
        ...
        try {
            ...
            const mockData = [{
                ...
                type: "apartment",
                description: "Beautiful studio apartment in the heart of the city with modern amenities and great location...",
                price: "1200.00",
                location: "Downtown, Cairo",
                space: "45.50",
                status: "published",
                primary_image: "https://images.unsplash.com/photo-1449844908441-8829872d2607?w=400&h=300&fit=crop",
                bedrooms: 1,
                bathrooms: 1,
                parking: 1,
    ...
    return ( <
        ...
        { /* Filter Section */ } <
        div className = "bg-white  shadow-sm p-4 md:p-6 mb-6" > { /* Top Row Filters */ } <
        div className = "grid grid-cols-1 md:grid-cols-3 gap-3 mb-4" >
        <
        div className = "relative" >
        <
        select className = "w-full p-2 pr-8 text-sm border border-gray-300  focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white text-gray-700 appearance-none cursor-pointer"
        value = { filters.type }
        onChange = {
        ...
        onChange = {
            (e) => handleFilterChange("location", e.target.value) } >
        <
        ...
        onChange = {
            (e) => handleFilterChange("bedrooms", e.target.value) }
        /> <
        /div>
        ...
        onChange = {
            (e) => handleFilterChange("minArea", e.target.value) }
        /> <
        /div>
        ...
        onChange = {
            (e) => handleFilterChange("maxArea", e.target.value) }
        /> <
        /div>
        ...
        { /* Error State */ } {
            error && !loading && ( <
                ...
                Showing demo data.Please check
                if the API server is running at http: //127.0.0.1:8000
                <
                /div> <
                /div>
            )
            ...
            !loading && ( <
                ...
                div className = "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6" > {
                    filteredProperties.map((property) => ( <
                        ...
                        <
                        span className = "bg-white text-gray-800 px-2 py-1 rounded text-xs font-medium" > { property.type } <
                        /span> <
                        /div> <
                        /div>
                        ...
                        { /* Title */ } <
                        h3 className = "font-bold text-base md:text-lg text-gray-900 mb-2" > { property.title } <
                        /h3>

                        { /* Description */ } <
                        p className = "text-gray-600 text-xs md:text-sm mb-4 line-clamp-2" > { property.description } <
                        /p>

                        { /* Price */ } <
                        div className = "mb-4" >
                        <
                        span className = "text-lg md:text-xl font-bold text-gray-900" > { property.formatted_price || `${property.price} EGP` } <
                        /span> {
                            property.price_per_sqm && ( <
                                span className = "text-xs text-gray-500 ml-2" > { property.price_per_sqm } <
                                /span>
                            )
                        } <
                        /div>
